...
describe('Pagination', () => {
  ...
  eyes.it('has correct inline layout', async () => {
    const pagination = paginationTestkitFactory({
    ...
    expect(nextLoc.x).toBeGreaterThan(
      pageSelectionLoc.x + pageSelectionSize.width,
    );
  });
  ...
  it('supports showFirstPage and showLastPage', () => {
    ...
    expect(pagination.getPageList()).toEqual([
      '1',
      '...',
      '4',
      '5',
  ...
  it('responds to rtl prop', async () => {
    ...
    expect(prevLoc.x).toBeGreaterThan(
      pageSelectionLoc.x + pageSelectionSize.width,
    );
