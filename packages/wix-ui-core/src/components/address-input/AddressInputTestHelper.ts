...
export const GEOCODE_1: Geocode = createGeocode(
  ADDRESS_1.place_id,
  ADDRESS_1.description,
);
export const GEOCODE_2: Geocode = createGeocode(
  ADDRESS_2.place_id,
  ADDRESS_2.description,
);
export const PLACE_DETAILS_1: PlaceDetails = createPlaceDetails(
  ADDRESS_1.place_id,
  ADDRESS_1.description,
);
export const PLACE_DETAILS_2: PlaceDetails = createPlaceDetails(
  ADDRESS_2.place_id,
  ADDRESS_2.description,
);
...
export const waitForSingleOption = (option, driver) => {
  return waitForCond(() => {
    try {
      return (
        first(getOptionsText(driver)) === option &&
        driver.getOptionsCount() === 1
      );
    } catch (e) {
      return false;
    }
