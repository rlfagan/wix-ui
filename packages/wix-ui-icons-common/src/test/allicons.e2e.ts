...
import { CLASSIC_EDITOR_CATEGORY, GENERAL_ICONS, SYSTEM_ICONS, WSR_CATEGORY, ON_STAGE_CATEGORY, REACTIONS_ICONS } from '../../stories/constants'
...
describe("All icons", () => {
  ...
  eyes.it(
    `check On-Stage - Reactions Icons`,
    async () => {
      const storyUrl = getStoryUrl(ON_STAGE_CATEGORY, REACTIONS_ICONS);
      browser.get(storyUrl);
      await browser.wait(
        ExpectedConditions.presenceOf($(`[data-hook="icon-list"]`))
      );
